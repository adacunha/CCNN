...
TTTBoard::TTTBoard() : Board(TTT_BOARD_SIZE, TTT_BOARD_SIZE) {

}

bool TTTBoard::place_X(int row_i, int col_i){
	return this->place_piece(X, row_i, col_i);
}	

bool TTTBoard::place_O(int row_i, int col_i){
	return this->place_piece(O, row_i, col_i);
}

bool TTTBoard::O_victor(){
	return consecutive_count(O, 3);		
}			

bool TTTBoard::X_victor(){
	return consecutive_count(X, 3);
}

bool TTTBoard::is_drawn(){
	if(this->O_victor() || this->X_victor()) return false;
	return this->get_piece_count() == TTT_BOARD_SIZE*TTT_BOARD_SIZE;
}

std::vector<std::vector<std::vector<int>>> TTTBoard::get_empty_cnn_input(){
	std::vector<std::vector<std::vector<int>>> layer(2, std::vector<std::vector<int>>(TTT_BOARD_SIZE, std::vector<int>(TTT_BOARD_SIZE, 0)));
	return layer;
}

void TTTBoard::fill_cnn_input_X(std::vector<std::vector<std::vector<int>>>& input_layer){
	fill_cnn_input(X, input_layer);
}

void TTTBoard::fill_cnn_input_O(std::vector<std::vector<std::vector<int>>>& input_layer){
	fill_cnn_input(O, input_layer);
}

void TTTBoard::print_layer(std::vector<std::vector<std::vector<int>>>& layer){
	std::cout << std::endl;
...
void TTTBoard::print_cnn_layers(){
...
bool TTTBoard::consecutive_count(Piece p, int count){
...
void TTTBoard::fill_cnn_input(Piece player_piece, std::vector<std::vector<std::vector<int>>>& input_layer){
